...
export default function LandingPage({ handleGoogleSignIn, handleGuestSignIn, isSigningIn }) {
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-4 font-sans relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        ...
        <div className="absolute top-[-20%] left-[-10%] w-[60vw] h-[60vw] rounded-full bg-amber-500/40 blur-[120px] mix-blend-screen animate-pulse-slow"></div>
        <div className="absolute bottom-[-10%] right-[-10%] w-[70vw] h-[70vw] rounded-full bg-orange-600/30 blur-[150px] mix-blend-screen animate-pulse-slower"></div>
      ...
      <div className="max-w-[460px] w-full bg-zinc-900/40 backdrop-blur-3xl rounded-[2.5rem] shadow-2xl border border-white/20 p-10 text-center space-y-10 animate-in relative z-10 transition-all duration-700 hover:shadow-amber-500/20 hover:border-white/30 group">

        {/* Animated Icon Container */}
        <div className="relative mx-auto w-24 h-24 flex items-center justify-center">
          <div className="absolute inset-0 bg-gradient-to-tr from-amber-400 to-orange-600 rounded-3xl blur-xl opacity-50 group-hover:opacity-80 transition-opacity duration-500 animate-spin-slow"></div>
          <div className="w-20 h-20 bg-gradient-to-tr from-zinc-800 to-zinc-900 border border-white/10 rounded-3xl flex items-center justify-center shadow-2xl text-amber-500 transform rotate-3 group-hover:rotate-12 transition-transform duration-500 relative z-10">
            <BookOpen size={40} className="drop-shadow-lg" />
            <Sparkles size={16} className="absolute -top-2 -right-2 text-amber-300 animate-pulse" />
          </div>
        </div>

        {/* Typography */}
        <div className="space-y-4">
          <h1 className="text-5xl font-serif font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-white to-amber-200 tracking-tight leading-tight">
            NovelQuest
          </h1>
          <p className="text-sm font-medium text-zinc-400 mt-2 max-w-[320px] mx-auto leading-relaxed">
            Your ultimate interactive novel companion. Immerse yourself in stories, explore deep character arcs, uncover hidden lore, and weave new narrative threads.
          </p>
        </div>
